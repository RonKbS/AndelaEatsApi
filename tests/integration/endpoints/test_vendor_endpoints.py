...
class TestVendorEndpoints(BaseTestCase):
    ...
    def test_delete_vendor_endpoint_with_right_permission(self):
        ...
        permission = PermissionFactory.create(keyword='delete_vendor', role=role)
    ...
    def test_list_suspended_vendors(self):
        location = LocationFactory(id=self.headers()['X-Location'])
        vendors = VendorFactory.create_batch(3, is_active=True, location=location)
        vendors = VendorFactory.create_batch(4, is_active=False, location=location)
    ...
    def test_list_deleted_vendors(self):
        location = LocationFactory(id=self.headers()['X-Location'])
        vendors = VendorFactory.create_batch(4, is_deleted=True, location=location)
        vendors = VendorFactory.create_batch(3, is_deleted=False, location=location)
